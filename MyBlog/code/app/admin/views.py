...
from flask import render_template, redirect, flash, \
    url_for, request, current_app, jsonify,send_file
...
from .forms import SubmitArticlesForm, ManageArticlesForm, DeleteArticleForm, \
    ...
    CustomBlogInfoForm, AddBlogPluginForm, ChangePasswordForm, EditUserInfoForm,ImportFileForm
from .. import db
...
def custom_blog_info():
    ...
    if form.validate_on_submit():
        ...
        f.save('app/static/images/background.jpg')
...
#TODO need to add the import method
@admin.route('/data_importexport/',methods=['GET','POST'])
@login_required
def data_importexport():
    form = ImportFileForm()
    if form.validate_on_submit():
        for each in form.fileupload.data.read():
           print(each)
        flash("导入成功！",'success')
        return redirect(url_for('admin.data_importexport'))
    return render_template('admin/blog_import_export.html',form=form)


@admin.route('/data_export/')
@login_required
def data_export():
    data = (x.tojson() for x in Article.query.all())
    output = ""
    for each in data:
        output += (str(each)+os.linesep)
    with open('app/temp.json',mode='w') as a:
        a.write(output)
    return send_file('temp.json', as_attachment=True, mimetype='text/plain')
