...
@main.route('/')
def index():
    BlogView.add_view(db,request.remote_addr)
...
@main.route('/article-contents/',methods=['GET'])
def articleContents():
    contents = request.args.get('keywords', None)
    keywords = '%{0}%'.format(contents)
    BlogView.add_view(db,request.remote_addr)
    page = request.args.get('page', 1, type=int)
    pagination = Article.query.filter(or_(Article.title.like(keywords),Article.summary.like(keywords))).order_by(
        Article.create_time.desc()).paginate(
        page, per_page=current_app.config['ARTICLES_PER_PAGE'],
        error_out=False)
    articles = pagination.items
    ...
    return render_template('index.html', articles=articles,
                           pagination=pagination, endpoint='.articleContents',keywords=contents)
